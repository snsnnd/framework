...
- **模板库 / 组件市场感**：左侧面板以中文类型名和分类配色展示 HAL、Sensor、Actuator、Algorithm、Module、Task 和自定义卡片模板，后续可以继续扩展成插件式市场。
- **分类配色**：画布卡片、端口和连线会按 HAL、Sensor、Actuator、Algorithm、Module、Task 等类型使用不同颜色，降低大型 Graph 的阅读成本。
